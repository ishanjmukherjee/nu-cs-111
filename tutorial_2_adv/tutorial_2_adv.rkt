...
(define (evaluate-primitive-expression exp)
  (lookup exp dictionary))

(define (evaluate exp)
  (if (list? exp)
      (evaluate-complex-expression exp)
      (evaluate-primitive-expression exp)))

(define (evaluate-complex-expression exp)
  (evaluate-procedure-call exp))

(define (evaluate-procedure-call exp)
  (local [(define results (map (λ (subexp) (evaluate subexp)) exp))]
